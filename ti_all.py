...
ka3 = 40.517
int_free =0
free = open("freedist.dat", "w")
freeang = open("freeang.dat", "w")
freeall = open("freeall.dat", "w")
...
#    du=np.zeros(num+1)
    ...
    for ts in u.trajectory:
        ...
        du1[count] = 0.5*kb*((dist[count]-1.5080)**2)
        dusum1 = du1[count] + dusum1
...
#        du[count] = 0.5*ka2*((theta[count]- np.deg2rad(109.6080))**2)
    ...
    hist, bin_edges = np.histogram(dist, bins=numbin, range=(0,10), density=True)
#    hi_du, bin_du = np.histogram(du, bins=1000, range=(0,100))

#### loop for managing the output for all angles ##
    lam = float(i)/10.0
    for p in range(0,6):
...
#        file.write(str(bin_edges[j])+" "+str(hist[j])+"\n")
 #       file1.write(str(bin_edges[j])+" "+str(hi)+"\n")
    avdusum = dusum1/float(num)
#    lam = float(i)/10.0

### output the bond free energy ###
    for j in range(1, 1000, 1):
        file10.write(str(bin_edges[j])+" "+str(hist[j])+"\n")
        
    free.write(str(lam)+" "+str(avdusum)+"\n")
#    int_free = (avdusum*0.1)+int_free

    freeang.write(str(lam)+" "+str(sumsum)+"\n")
    freeall.write(str(lam)+" "+str(avdusum+sumsum)+"\n")
